...
if ($action === 'login_otp_resend') {
  ...
  if (!$pending) gsm_send(false, 'No pending OTP request.', null, 400);
  ...
  if ($pEmail === '' || $purpose === '') gsm_send(false, 'No pending OTP request.', null, 400);
  $sent = otp_send($db, $pEmail, $purpose, 180);
  if (!($sent['ok'] ?? false)) gsm_send(false, (string)($sent['message'] ?? 'Failed to send OTP.'), $sent['data'] ?? null, 500);
...
if ($action === 'login_otp_verify') {
  ...
  if ($pEmail === '' || $purpose === '' || $deviceHash === '' || $userType === '') gsm_send(false, 'Invalid OTP request.', null, 400);
  if (strlen($code) !== 6) gsm_send(false, 'Invalid OTP.', null, 400);

  $ver = otp_verify($db, $pEmail, $purpose, $code);
  if (!($ver['ok'] ?? false)) gsm_send(false, (string)($ver['message'] ?? 'Invalid or expired OTP.'), null, 401);

  if ($userType === 'rbac') {
    $userId = (int)($pending['user_id'] ?? 0);
    if ($userId <= 0) gsm_send(false, 'Invalid OTP request.', null, 400);
    $user = rbac_get_user_by_email($db, $pEmail);
    if (!$user || (int)($user['id'] ?? 0) !== $userId) gsm_send(false, 'Invalid OTP request.', null, 400);
  ...
  if ($userType === 'operator') {
    ...
    if ($opUserId <= 0 || $plate === '') gsm_send(false, 'Invalid OTP request.', null, 400);
...
if ($action === 'operator_login') {
  ...
  if ($plateNumber === '') gsm_send(false, 'Plate number is required', null, 400);
  if ($password === '') gsm_send(false, 'Password is required', null, 400);
  if ($deviceId === '') gsm_send(false, 'Device verification failed. Please refresh and try again.', null, 400);
