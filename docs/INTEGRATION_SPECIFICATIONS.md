...
## 4. Traffic Congestion & Incident Intelligence (TomTom)
**Purpose:** To enrich route and terminal AI insights using live congestion and incident data.

### A. Traffic Flow & Incidents (Pulling Data)
*   **Trigger Module:** `Dashboard` > `AI Insights`
*   **Trigger Event:** When generating high-demand alerts and dispatch playbooks.
*   **Data to REQUEST (TMM → TomTom):**
    1. **Geocoding** (resolve a terminal address/city or route origin/destination to coordinates)
       - Endpoint: `GET /search/2/geocode/{query}.json`
    2. **Traffic Flow** (congestion at a point)
       - Endpoint: `GET /traffic/services/4/flowSegmentData/absolute/10/json?point={lat,lon}`
    3. **Incident Details** (active incidents near a point)
       - Endpoint: `GET /traffic/services/5/incidentDetails?bbox={minLon,minLat,maxLon,maxLat}&timeValidityFilter=present`

*   **Key Notes:**
    - API key is stored in `app_settings` as `tomtom_api_key` (no hardcoded secrets).
    - Terminals are resolved using `terminals.address + terminals.city`.
    - Routes are resolved using `routes.origin` and `routes.destination`.

*   **Data to USE (TomTom → TMM):**
    - `currentSpeed` vs `freeFlowSpeed` → congestion classification (free/moderate/heavy/severe).
    - `incidents[]` → incident count + top descriptions shown in AI Insights.
