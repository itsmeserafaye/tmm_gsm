...
CREATE TABLE IF NOT EXISTS inspection_schedules (
  ...
  status ENUM('Scheduled','Completed','Cancelled','Rescheduled','Overdue / No-Show','Pending Verification','Pending Assignment') DEFAULT 'Pending Verification',
  cr_verified TINYINT(1) DEFAULT 0,
  or_verified TINYINT(1) DEFAULT 0,
  status_remarks VARCHAR(255) DEFAULT NULL,
  overdue_marked_at DATETIME DEFAULT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX (plate_number),
  INDEX (vehicle_id),
  FOREIGN KEY (plate_number) REFERENCES vehicles(plate_number) ON DELETE CASCADE,
