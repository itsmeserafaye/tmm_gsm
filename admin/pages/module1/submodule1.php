...
<div class="mx-auto max-w-7xl px-4 sm:px-6 md:px-8 mt-6 font-sans text-slate-900 dark:text-slate-100 space-y-8">
  <div
    ...
    <div>
      ...
      <p class="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-2xl">Manage PUV master records, OR/CR document
        storage, ownership details, transfers, and status tracking.</p>
    </div>
  </div>
  ...
  <div
    class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between bg-white dark:bg-slate-800 p-5 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
    <form id="filterForm" class="flex flex-1 flex-col gap-4 sm:flex-row sm:items-center" method="GET">
      ...
      <div class="relative w-full sm:max-w-xs group">
        <div
          class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 transition-colors group-focus-within:text-blue-500">
          <i data-lucide="search" class="h-4 w-4 text-slate-400"></i>
        </div>
        <input name="q" id="searchInput"
          class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 pl-11 pr-4 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600 placeholder:text-slate-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-semibold text-sm"
          placeholder="Search plate or operator..." value="<?php echo htmlspecialchars($_GET['q'] ?? ''); ?>">
      </div>

      <?php require_once __DIR__ . '/../../includes/vehicle_types.php';
      $types = vehicle_types(); ?>
      <!-- Type Select -->
      <div class="w-full sm:w-48">
        <div class="relative">
          <select name="vehicle_type" id="typeSelect"
            class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 pl-4 pr-10 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-semibold text-sm appearance-none cursor-pointer">
      ...
      <div class="w-full sm:w-48">
        <div class="relative">
          <select name="status" id="statusSelect"
            ...
            <option value="Suspended" <?php echo (($_GET['status'] ?? '') === 'Suspended') ? 'selected' : ''; ?>>Suspended
            </option>
            <option value="Deactivated" <?php echo (($_GET['status'] ?? '') === 'Deactivated') ? 'selected' : ''; ?>>
              Deactivated</option>
          </select>
          <i data-lucide="chevron-down"
            class="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none"></i>
    ...
    <div id="tableActionsToolbar"
      class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 border-t lg:border-t-0 border-slate-200 dark:border-slate-700 pt-4 lg:pt-0">
      <button id="openUploadDocsModalBtn" type="button"
        class="inline-flex justify-center items-center gap-x-2 rounded-md bg-white dark:bg-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-200 shadow-sm border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 transition-all">
        <i data-lucide="upload-cloud" class="-ml-0.5 h-4 w-4 text-slate-400"></i>
        Upload Docs
      </button>
      <button id="openCreateVehicleModalBtn" type="button"
        class="inline-flex justify-center items-center gap-x-2 rounded-md bg-blue-700 hover:bg-blue-800 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all active:scale-[0.98]">
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
        <thead class="bg-slate-50 dark:bg-slate-700">
          <tr>
            <th scope="col"
              class="py-4 pl-4 sm:pl-6 pr-3 text-left text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
              Plate</th>
            <th scope="col"
              class="px-3 py-4 text-left text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
              Type</th>
            <th scope="col"
              class="px-3 py-4 text-left text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
              Operator / COOP</th>
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-800">
          ...
          $typesStr = '';
          if ($q !== '') {
            $conds[] = "(v.plate_number LIKE ? OR v.operator_name LIKE ?)";
            $params[] = "%$q%";
            $params[] = "%$q%";
            $typesStr .= 'ss';
          }
          ...
          while ($row = $res->fetch_assoc()):
            ...
            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors group">
              <td class="whitespace-nowrap py-4 pl-4 sm:pl-6 pr-3">
                <div class="font-black text-sm text-slate-800 dark:text-white">
                  <?php echo htmlspecialchars($row['plate_number']); ?></div>
              </td>
              <td class="whitespace-nowrap px-3 py-4 text-sm text-slate-500 dark:text-slate-400">
                <span
                  class="inline-flex items-center rounded-lg bg-slate-100 dark:bg-slate-700/50 px-2.5 py-1 text-xs font-bold text-slate-600 dark:text-slate-300 ring-1 ring-inset ring-slate-500/10">
                  <?php echo htmlspecialchars($row['vehicle_type']); ?>
                </span>
              </td>
              <td class="px-3 py-4 text-sm text-slate-500 dark:text-slate-400">
                <div class="font-bold text-slate-700 dark:text-slate-200">
                  <?php echo htmlspecialchars($row['operator_name']); ?></div>
                <?php if ($row['coop_name']): ?>
                  <div class="text-xs font-medium text-slate-400 mt-0.5"><?php echo htmlspecialchars($row['coop_name']); ?>
                  </div><?php endif; ?>
              </td>
              <td class="px-3 py-4 text-sm text-slate-500 dark:text-slate-400">
                <div class="flex flex-col gap-1">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <?php if (!empty($row['route_id'])): ?>
                      <a class="text-indigo-600 dark:text-indigo-400 font-bold hover:underline"
                        href="?page=module1/submodule3&route_id=<?php echo urlencode((string) $row['route_id']); ?>&plate=<?php echo urlencode((string) $row['plate_number']); ?>">
              ...
              <td class="whitespace-nowrap px-3 py-4 text-sm">
                ...
                <div class="flex items-center gap-2">
                  <span
                    class="inline-flex items-center rounded-lg px-2.5 py-1 text-xs font-bold ring-1 ring-inset <?php echo $badgeClass; ?>">
                  ...
                  <?php if ($statusHint !== ''): ?>
                    <div class="group/hint relative">
                      ...
                      <div
                        class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-900 text-white text-xs rounded-lg shadow-xl opacity-0 group-hover/hint:opacity-100 transition-opacity pointer-events-none z-10">
              ...
              <td class="relative whitespace-nowrap py-4 pl-3 pr-6 text-right text-sm font-medium">
                <div
                  ...
                  <button title="Transfer Ownership"
                    ...
                    class="p-2 rounded-md bg-slate-100 dark:bg-slate-700/50 text-slate-500 hover:text-orange-700 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
  ...
  <div id="loading-indicator" class="fixed top-6 right-6 z-50 hidden transition-all duration-300">
    <div
      class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md text-slate-700 dark:text-slate-200 px-4 py-2.5 rounded-md shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-3">
  ...
  <div id="createVehicleModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"></div>
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
      <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div
          class="relative transform overflow-hidden rounded-2xl bg-white dark:bg-slate-900 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-slate-200 dark:border-slate-700">
          <!-- Header -->
          <div
            class="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-700">
            <h3 class="text-lg font-bold leading-6 text-slate-900 dark:text-white" id="modal-title">New Vehicle Record
            </h3>
            <button id="createVehicleModalClose"
              class="p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-300 transition-colors">
          ...
          <form id="createVehicleForm" class="p-6 sm:p-8 space-y-6" novalidate method="POST"
            ...
            <div>
              ...
              <div class="relative group">
                <input name="plate_number" id="cv_plate"
                  ...
                  placeholder="ABC-1234" pattern="^[A-Z0-9-]{6,10}$" required>
                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 hidden"
                  id="cv_plate_error_icon">
                  <i data-lucide="alert-circle" class="h-5 w-5 text-red-500"></i>
                </div>
              </div>
              <p class="mt-2 text-xs font-bold text-red-500 hidden" id="cv_plate_error">Invalid plate format (e.g.,
                ABC-1234)</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label
                  class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Type</label>
                <div class="relative">
                  <?php $types = vehicle_types(); ?>
                  <select name="vehicle_type"
                    class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 pl-4 pr-10 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none font-semibold text-sm appearance-none cursor-pointer transition-all"
                    required>
              ...
              <div>
                <label
                  class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Status</label>
                <div
                  class="block w-full rounded-xl border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 py-3.5 px-4 text-xs font-medium text-slate-500 dark:text-slate-400">
            ...
            <div>
              <label
                class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Operator
                Name</label>
              <input name="operator_name" value="<?php echo htmlspecialchars($prefillOperator); ?>"
                class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-4 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600 placeholder:text-slate-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none font-semibold transition-all"
            ...
            <div class="grid grid-cols-2 gap-6">
              <div>
                <label
                  class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Franchise
                  No.</label>
                <div>
                  <?php
                  $frList = [];
                  ...
                  ?>
                  <input name="franchise_id" value="<?php echo htmlspecialchars($prefillFrRef); ?>" list="franchiseList"
                    class="block w-full rounded-md border-0 bg-slate-50 dark:bg-slate-900/50 py-2.5 px-4 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-slate-200 dark:ring-slate-700 placeholder:text-slate-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 font-semibold transition-all uppercase"
                    placeholder="Select existing franchise (e.g., 2024-00123)">
              ...
              <div>
                <label
                  class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Route
                  Assignment</label>
                <div
                  class="block w-full rounded-md border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 py-2.5 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400">
            ...
            <div class="mt-8 flex items-center justify-end gap-3 pt-6 border-t border-slate-200 dark:border-slate-700">
              <button type="button" id="createCancelBtn"
                class="px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-md transition-colors">Cancel</button>
              <button type="submit" id="btnSaveVehicle"
                class="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-blue-700 hover:bg-blue-800 rounded-md shadow-sm active:scale-[0.98] transition-all">
  ...
  <!-- Upload Docs Modal -->
  <div id="uploadDocsModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"></div>
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
      <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div
          ...
          <div
            class="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-700">
            <h3 class="text-lg font-bold leading-6 text-slate-900 dark:text-white" id="modal-title">Upload Documents
            </h3>
            <button id="uploadDocsModalClose"
              class="p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-300 transition-colors">
              <i data-lucide="x" class="h-5 w-5"></i>
            </button>
          </div>
          ...
          <form id="uploadDocsForm" class="p-8 space-y-6" method="POST" action="/tmm/admin/api/module1/upload_docs.php"
            ...
            <div>
              <label
                class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Target
                Vehicle</label>
              <div class="relative group">
                <input name="plate_number" id="plateSearchInput" list="platesList"
                  class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-4 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600 placeholder:text-slate-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none font-semibold transition-all uppercase"
                  placeholder="Search Plate..." autocomplete="off" required>
            ...
            <div class="space-y-4">
              <div class="group">
                <label
                  class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">OR
                  Document</label>
                <input name="or" type="file" accept=".pdf,.jpg,.jpeg,.png"
                  class="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:uppercase file:tracking-wide file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-blue-900/20 dark:file:text-blue-400 transition-all cursor-pointer bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
              </div>
              <div class="group">
                <label
                  class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">CR
                  Document</label>
              ...
              <div class="group">
                <label
                  class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Deed
                  of Sale</label>
                <input name="deed" type="file" accept=".pdf,.jpg,.jpeg,.png"
                  class="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:uppercase file:tracking-wide file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-blue-900/20 dark:file:text-blue-400 transition-all cursor-pointer bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
              </div>
            </div>
            ...
            <div class="mt-8 flex items-center justify-end gap-3 pt-6 border-t border-slate-200 dark:border-slate-700">
              <button type="button" id="uploadCancelBtn"
                class="px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-md transition-colors">Cancel</button>
              <button type="submit" id="btnUploadDocs"
                class="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-blue-700 hover:bg-blue-800 rounded-md shadow-sm active:scale-[0.98] transition-all">
  ...
  <div id="transferModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"></div>
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
      <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div
          class="relative transform overflow-hidden rounded-2xl bg-white dark:bg-slate-900 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-slate-200 dark:border-slate-700">
          <div
            class="bg-slate-50/50 dark:bg-slate-800/50 px-6 py-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-700">
            <h3 class="text-lg font-black leading-6 text-slate-900 dark:text-white" id="modal-title">Transfer Ownership
            </h3>
            <button id="transferModalClose"
              class="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-slate-500 transition-all">
              <i data-lucide="x" class="h-5 w-5"></i>
            </button>
          </div>
          ...
          <form id="transferModalForm" class="p-6 sm:p-8 space-y-6" method="POST"
            ...
            <div
              class="bg-orange-50 dark:bg-orange-900/10 p-5 rounded-2xl border border-orange-100 dark:border-orange-900/20">
              <div class="text-xs font-bold text-orange-600 dark:text-orange-400 uppercase tracking-widest mb-1">Vehicle
                Plate</div>
              <div id="transferModalPlateLabel"
                class="text-3xl font-black text-orange-700 dark:text-orange-300 uppercase tracking-tight"></div>
            </div>

            <div>
              <label
                class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">New
                Operator Name</label>
              <div class="relative">
                <input name="new_operator_name"
                  class="block w-full rounded-xl border-0 bg-slate-50 dark:bg-slate-800/50 py-3.5 px-4 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-slate-200 dark:ring-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-orange-500 font-bold transition-all"
                  placeholder="Full Name" required>
              </div>
            </div>
            ...
            <div>
              <label
                class="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Deed
                of Sale Reference</label>
              <div class="relative">
                <input name="deed_ref" id="deedRefInput"
                  class="block w-full rounded-xl border-0 bg-slate-50 dark:bg-slate-800/50 py-3.5 px-4 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-slate-200 dark:ring-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-orange-500 font-bold transition-all"
                  placeholder="Doc Ref #" autocomplete="off" required>
              </div>
            </div>

            <div class="mt-8 flex items-center justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
              <button type="button" id="transferCancelBtn"
                class="px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors">Cancel</button>
              <button type="submit" id="btnTransferModal"
                class="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-amber-600 hover:bg-amber-700 rounded-md shadow-sm active:scale-[0.98] transition-all">
                <span>Transfer</span>
  ...
  <div id="vehicleModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
    <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"></div>
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
      <div class="flex min-h-full items-center justify-center p-4">
        <div
          class="relative transform overflow-hidden rounded-2xl bg-white dark:bg-slate-900 text-left shadow-2xl transition-all sm:w-full sm:max-w-5xl border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]">
          <div
            class="bg-slate-50/50 dark:bg-slate-800/50 px-6 py-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
            <h3 class="text-lg font-black leading-6 text-slate-900 dark:text-white">Vehicle Details</h3>
            <button id="vehicleModalClose"
              class="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-slate-500 transition-all">
              <i data-lucide="x" class="h-5 w-5"></i>
            </button>
          </div>
  ...
  <script>
    (function () {
      ...
      function showToast(msg, type = 'success') {
        ...
        toast.className = `${colors} px-4 py-3 rounded-xl shadow-lg flex items-center gap-3 transform transition-all duration-300 translate-y-10 opacity-0 w-full sm:w-auto sm:min-w-[320px] backdrop-blur-md`;
        toast.innerHTML = `
      ...
      function setupTransferDeedAutofill() {
        ...
        if (!plateInput || !deedInput) return;
