  ...
  <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between border-b border-slate-200 dark:border-slate-700 pb-6">
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
      ...
      <?php if (has_permission('reports.export')): ?>
        ...
        <a href="<?php echo htmlspecialchars($rootUrl); ?>/admin/api/module1/export_operators_csv.php?<?php echo http_build_query(['q'=>$q,'operator_type'=>$type,'status'=>$status,'format'=>'excel']); ?>"
          class="inline-flex items-center gap-2 rounded-md bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/40 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-600 transition-colors">
          ...
          <?php if ($res && $res->num_rows > 0): ?>
            <?php while ($row = $res->fetch_assoc()): ?>
              <?php
                ...
                $st = $stRaw === 'Pending Validation' ? 'Incomplete' : ($stRaw === 'Rejected' ? 'Returned' : $stRaw);
                $badge = match($st) {
                  ...
                  'Incomplete' => 'bg-amber-100 text-amber-700 ring-amber-600/20 dark:bg-amber-900/30 dark:text-amber-400 dark:ring-amber-500/20',
