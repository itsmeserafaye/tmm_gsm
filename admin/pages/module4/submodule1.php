  ...
  if ($resExport) {
    while ($r = $resExport->fetch_assoc()) {
      ...
      if ($inspLabel !== '') {
        ...
        $inspParts = [];
        if (!empty($r['inspector_name'])) $inspParts[] = $r['inspector_name'];
        if (!empty($r['badge_no'])) $inspParts[] = $r['badge_no'];
...
<div class="mx-auto max-w-7xl px-4 sm:px-6 md:px-8 mt-6 font-sans text-slate-900 dark:text-slate-100 space-y-8">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-2 border-b border-slate-200 dark:border-slate-700 pb-6">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage vehicle inspections, verify documents (OR/CR), and assign inspectors.</p>
        </div>
    </div>
                    ...
                    <?php if ($vehicleInfo): ?>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                            <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                                <div class="text-[10px] uppercase font-bold tracking-wider text-slate-400 mb-1">Plate Number</div>
                                <div class="text-xl font-black text-slate-800 dark:text-white"><?php echo htmlspecialchars($vehicleInfo['plate_number'] ?? '', ENT_QUOTES); ?></div>
                            </div>
                            <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                                <div class="text-[10px] uppercase font-bold tracking-wider text-slate-400 mb-1">Operator</div>
                                <div class="font-bold text-slate-700 dark:text-slate-200 truncate"><?php echo htmlspecialchars($vehicleInfo['operator_name'] ?? '—', ENT_QUOTES); ?></div>
                            </div>
                            <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                                ...
                                <?php
                                  ...
                                  if ($inspStatus === 'PASSED') $badgeClass = 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400';
                                  if ($inspStatus === 'FAILED') $badgeClass = 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400';
                                ?>
                                <span class="inline-flex items-center rounded-lg px-2 py-1 text-xs font-bold <?php echo $badgeClass; ?>"><?php echo htmlspecialchars($labelText, ENT_QUOTES); ?></span>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <!-- OR Document -->
                            <div class="relative group p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700 transition-all">
                                <div class="flex justify-between items-start mb-2">
                                    <div class="font-bold text-slate-700 dark:text-slate-200">OR Document</div>
                                    <?php if ($orDoc): ?>
                                        <span class="px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider <?php echo ((int)($orDoc['verified'] ?? 0) === 1) ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'; ?>">
                                            <?php echo ((int)($orDoc['verified'] ?? 0) === 1) ? 'Verified' : 'Uploaded'; ?>
                                        </span>
                                    <?php else: ?>
                                        <span class="px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-rose-100 text-rose-700">Missing</span>
                                    <?php endif; ?>
                                </div>
                                <?php if ($orDoc): ?>
                            ...
                            <div class="relative group p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700 transition-all">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <?php if ($crDoc): ?>
                                        <span class="px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider <?php echo ((int)($crDoc['verified'] ?? 0) === 1) ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'; ?>">
                        ...
                        <?php if (!$orDoc || !$crDoc): ?>
                            <div class="mt-6 p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/30 border border-dashed border-slate-300 dark:border-slate-600">
                                ...
                                <form id="doc-upload-form" class="space-y-4" enctype="multipart/form-data">
                                    <input type="hidden" name="plate_number" value="<?php echo htmlspecialchars($vehicleInfo['plate_number'] ?? '', ENT_QUOTES); ?>">
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <?php if (!$orDoc): ?>
                                            <div>
                                                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">OR File</label>
                                                <input name="or" type="file" accept=".pdf,.jpg,.jpeg,.png" class="block w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all">
                                            </div>
                                        <?php endif; ?>
                                        <?php if (!$crDoc): ?>
                                            <div>
                                                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">CR File</label>
                                                <input name="cr" type="file" accept=".pdf,.jpg,.jpeg,.png" class="block w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all">
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <div id="doc-upload-status" class="text-xs font-medium text-slate-500"></div>
                                        <button type="submit" id="btn-doc-upload" class="px-4 py-2 rounded-xl bg-slate-800 dark:bg-white text-white dark:text-slate-900 text-xs font-bold hover:bg-slate-700 dark:hover:bg-slate-100 transition-all">Upload Files</button>
                                    </div>
                                </form>
                            </div>
                        <?php endif; ?>
                        ...
                        <?php if ($selectedSchedule && (string)($selectedSchedule['plate_number'] ?? '') === (string)($vehicleInfo['plate_number'] ?? '')): ?>
                            <div class="mt-6 border-t border-slate-100 dark:border-slate-700 pt-6">
                                <div class="flex items-center justify-between mb-4">
                                    <div>
                                        <h3 class="text-sm font-black text-slate-800 dark:text-white">Verify for Schedule #<?php echo (int)$selectedSchedule['schedule_id']; ?></h3>
                                        <p class="text-xs text-slate-500"><?php echo htmlspecialchars(substr((string)($selectedSchedule['scheduled_at'] ?? ''), 0, 16), ENT_QUOTES); ?></p>
                                    </div>
                                    <span class="px-3 py-1 rounded-xl text-xs font-bold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">
                                        <?php echo htmlspecialchars((string)($selectedSchedule['status'] ?? ''), ENT_QUOTES); ?>
                                    </span>
                                </div>
                                <form method="POST" class="flex flex-wrap items-center gap-4 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl">
                                    <input type="hidden" name="action" value="update_doc_verification">
                                    <input type="hidden" name="schedule_id" value="<?php echo (int)$selectedSchedule['schedule_id']; ?>">
                                    
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="cr_verified" value="1" <?php echo ((int)($selectedSchedule['cr_verified'] ?? 0) === 1) ? 'checked' : ''; ?> class="w-5 h-5 rounded-lg border-slate-300 text-blue-600 focus:ring-blue-500 transition-all">
                                        <span class="text-sm font-bold text-slate-700 dark:text-slate-200">CR Verified</span>
                                    </label>
                                    
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="or_verified" value="1" <?php echo ((int)($selectedSchedule['or_verified'] ?? 0) === 1) ? 'checked' : ''; ?> class="w-5 h-5 rounded-lg border-slate-300 text-blue-600 focus:ring-blue-500 transition-all">
                                        <span class="text-sm font-bold text-slate-700 dark:text-slate-200">OR Verified</span>
                                    </label>

                                    <div class="flex-1"></div>
                                    <button type="submit" class="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold shadow-lg shadow-blue-500/20 transition-all">
                                        Update Status
                                    </button>
                                </form>
                            </div>
                        <?php else: ?>
                            <div class="mt-6 p-4 rounded-2xl bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 text-center">
                                <p class="text-xs font-medium text-blue-600 dark:text-blue-400">Select an upcoming schedule from the list to manage verification.</p>
                            </div>
                        <?php endif; ?>

                    <?php else: ?>
                        <div class="py-12 text-center">
                            <div class="mx-auto h-16 w-16 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center mb-4">
            ...
            <div class="overflow-hidden rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm">
                <div class="relative p-6">
                    ...
                    <?php if ($scheduleError !== ''): ?>
                        <div class="mb-6 p-4 rounded-md bg-rose-50 border border-rose-200 text-rose-700 text-sm font-semibold">
                            <?php echo htmlspecialchars($scheduleError, ENT_QUOTES); ?>
                        </div>
                    <?php elseif ($scheduleMessage !== ''): ?>
                        <div class="mb-6 p-4 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-semibold">
                    ...
                    <form method="POST" id="schedule-form" class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-300 mb-2">Schedule Date</label>
                                <input type="datetime-local" name="scheduled_at" class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-4 text-slate-900 dark:text-white font-semibold border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-300 mb-2">Inspection Site</label>
                            <div class="relative">
                                <input id="schedule-location" name="location" class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-4 text-slate-900 dark:text-white font-semibold border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="e.g. City Inspection Yard • Lane A" autocomplete="off">
                                <div id="schedule-location-suggestions" class="absolute z-50 mt-1 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md shadow-xl text-xs max-h-48 overflow-y-auto hidden"></div>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-300 mb-2">Type</label>
                                <div class="relative">
                                    <select name="inspection_type" class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 pl-4 pr-10 text-slate-900 dark:text-white font-semibold border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none appearance-none transition-all">
                                        <?php $selType = $prefillType !== '' ? $prefillType : 'Annual'; ?>
                                        <option value="Annual" <?php echo $selType === 'Annual' ? 'selected' : ''; ?>>Annual Inspection</option>
                                        ...
                                        <option value="Special" <?php echo $selType === 'Special' ? 'selected' : ''; ?>>Special Inspection</option>
                                    </select>
                                    <i data-lucide="chevron-down" class="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none"></i>
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-300 mb-2">Requesting Office</label>
                                <input name="requested_by" class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-4 text-slate-900 dark:text-white font-semibold border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Optional">
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-300 mb-2">Assigned Inspector</label>
                            <div class="relative">
                                <input id="schedule-inspector" name="inspector_label" class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-4 text-slate-900 dark:text-white font-semibold border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Search Inspector Name or Badge ID" autocomplete="off">
                                <input type="hidden" name="inspector_id" id="schedule-inspector-id" value="">
                                <div id="schedule-inspector-suggestions" class="absolute z-50 mt-1 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md shadow-xl text-xs max-h-48 overflow-y-auto hidden"></div>
                            </div>
                            <div id="inspector-status" class="mt-2 text-xs font-medium text-slate-500 min-h-[1.25rem]"></div>
                        </div>

                        <div class="pt-2 flex justify-end">
                            <button type="submit" class="rounded-md bg-blue-700 hover:bg-blue-800 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors active:scale-[0.98]">
                                Create Schedule
                            </button>
                        </div>
                    </form>
                ...
                <div class="p-6 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <?php if ($upcomingSchedules): ?>
                            <a href="?page=module4/submodule1&q=<?php echo urlencode($search); ?>&status=<?php echo urlencode($statusFilter); ?>&insp=<?php echo (int)$inspectorFilter; ?>&insp_name=<?php echo urlencode($inspectorFilterName); ?>&scheduled_from=<?php echo urlencode($scheduledFrom); ?>&scheduled_to=<?php echo urlencode($scheduledTo); ?>&export=schedules" class="inline-flex items-center gap-1.5 px-3 py-2 rounded-md bg-white dark:bg-slate-800 text-sm font-semibold text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">
                    ...
                    <form method="GET" id="upcoming-filter-form" class="space-y-3">
                        <input type="hidden" name="page" value="module4/submodule1">
                        <input id="upcoming-filter-q" type="text" name="q" value="<?php echo htmlspecialchars($search, ENT_QUOTES); ?>" class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-3 text-sm font-semibold text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Filter by Plate, Location...">
                        
                        <div class="grid grid-cols-2 gap-3">
                            <select name="status" class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-3 text-sm font-semibold text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none appearance-none transition-all">
                                <option value="">All Status</option>
                                <option value="Scheduled" <?php echo $statusFilter === 'Scheduled' ? 'selected' : ''; ?>>Scheduled</option>
                                <option value="Completed" <?php echo $statusFilter === 'Completed' ? 'selected' : ''; ?>>Completed</option>
                                <option value="Cancelled" <?php echo $statusFilter === 'Cancelled' ? 'selected' : ''; ?>>Cancelled</option>
                            </select>
                            <input type="date" name="scheduled_from" value="<?php echo htmlspecialchars($scheduledFrom, ENT_QUOTES); ?>" class="block w-full rounded-md bg-white dark:bg-slate-900/50 py-2.5 px-3 text-sm font-semibold text-slate-900 dark:text-white border border-slate-200 dark:border-slate-600 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                        </div>
                    </form>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                    <?php if ($upcomingSchedules): ?>
                        <?php foreach ($upcomingSchedules as $row): ?>
                            <div class="group relative p-4 rounded-md bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/30 border border-slate-200 dark:border-slate-700 transition-colors">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                        <a href="?page=module4/submodule1&plate=<?php echo urlencode($row['plate_number'] ?? ''); ?>&schedule_id=<?php echo (int)$row['schedule_id']; ?>" class="text-sm font-black text-slate-800 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                                    ...
                                    <?php
                                      ...
                                      if ($st === 'Completed') $sc = 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400';
                                      if ($st === 'Cancelled') $sc = 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300';
                                    ?>
                                    <span class="px-2 py-1 rounded-lg text-[10px] font-bold <?php echo $sc; ?>"><?php echo htmlspecialchars($st ?: 'Scheduled', ENT_QUOTES); ?></span>
                                </div>
                                
                                <div class="flex items-center gap-2 text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                                ...
                                <?php if (!empty($row['location'])): ?>
                                    <div class="flex items-center gap-2 text-xs text-slate-500 mb-2">
                                        ...
                                        <span class="truncate"><?php echo htmlspecialchars($row['location'] ?? '', ENT_QUOTES); ?></span>
                                    </div>
                                <?php endif; ?>

                                <div class="pt-2 mt-2 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-between">
                                    <div class="flex items-center gap-1.5">
                                        <div class="h-5 w-5 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-[10px] font-bold text-slate-600 dark:text-slate-300">
                                            <i data-lucide="user" class="w-3 h-3"></i>
                                        </div>
                                        <span class="text-xs font-medium text-slate-600 dark:text-slate-300 truncate max-w-[120px]">
                                            <?php
                                              $label = trim((string)($row['inspector_label'] ?? ''));
                                              if ($label === '') {
                                                  $inspParts = [];
                                                  if (!empty($row['inspector_name'])) $inspParts[] = $row['inspector_name'];
                                                  if (!empty($row['badge_no'])) $inspParts[] = $row['badge_no'];
                                    ...
                                    <?php
                                      ...
                                      if ($hasQrRow) {
                                        ...
                                        echo '<button type="button" class="btn-show-qr p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all" data-qr-url="'.htmlspecialchars($qrUrlRow, ENT_QUOTES).'"><i data-lucide="qr-code" class="w-4 h-4"></i></button>';
                                      }
                                    ?>
                                </div>
                            </div>
                        ...
                        <?php if ($maxPage > 1): ?>
                            <div class="flex items-center justify-center gap-2 pt-4">
                                <?php if ($page > 1): ?>
                                    <a href="?page=module4/submodule1&q=<?php echo urlencode($search); ?>&status=<?php echo urlencode($statusFilter); ?>&insp=<?php echo (int)$inspectorFilter; ?>&insp_name=<?php echo urlencode($inspectorFilterName); ?>&scheduled_from=<?php echo urlencode($scheduledFrom); ?>&scheduled_to=<?php echo urlencode($scheduledTo); ?>&p=<?php echo $page - 1; ?>" class="p-2 rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-600 hover:bg-slate-200 transition-all">
                                        <i data-lucide="chevron-left" class="w-4 h-4"></i>
                                    </a>
                                <?php endif; ?>
                                <span class="text-xs font-bold text-slate-500">Page <?php echo $page; ?> of <?php echo $maxPage; ?></span>
                                <?php if ($page < $maxPage): ?>
                                    <a href="?page=module4/submodule1&q=<?php echo urlencode($search); ?>&status=<?php echo urlencode($statusFilter); ?>&insp=<?php echo (int)$inspectorFilter; ?>&insp_name=<?php echo urlencode($inspectorFilterName); ?>&scheduled_from=<?php echo urlencode($scheduledFrom); ?>&scheduled_to=<?php echo urlencode($scheduledTo); ?>&p=<?php echo $page + 1; ?>" class="p-2 rounded-md bg-slate-100 dark:bg-slate-700 text-slate-600 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
                    ...
                    <?php else: ?>
                        <div class="h-64 flex flex-col items-center justify-center text-center p-6">
                            <div class="h-16 w-16 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center mb-4">
...
<div id="qr-modal-overlay" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] hidden transition-opacity opacity-0">
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-6 w-80 transform transition-all scale-95 border border-slate-200 dark:border-slate-700">
        ...
        <div class="flex justify-center mb-6">
            <div class="p-4 rounded-md bg-white dark:bg-slate-900 shadow-inner border border-slate-200 dark:border-slate-700">
                <img id="qr-modal-image" src="" alt="Certificate QR" class="w-48 h-48 rounded-lg">
            </div>
        </div>
        <button type="button" id="qr-modal-close" class="w-full py-2.5 rounded-md bg-white dark:bg-slate-800 text-sm font-semibold text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">Close</button>
    </div>
</div>
