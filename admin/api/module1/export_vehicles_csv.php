...
fputcsv($out, ['vehicle_id','plate_number','vehicle_type','operator_id','operator_name','engine_no','chassis_no','make','model','year_model','fuel_type','color','route_id','status','created_at']);
while ($row = $res->fetch_assoc()) {
  fputcsv($out, [$row['vehicle_id'],$row['plate_number'],$row['vehicle_type'],$row['operator_id'],$row['operator_name'],$row['engine_no'],$row['chassis_no'],$row['make'],$row['model'],$row['year_model'],$row['fuel_type'],$row['color'],$row['route_id'],$row['status'],$row['created_at']]);
}
fclose($out);
