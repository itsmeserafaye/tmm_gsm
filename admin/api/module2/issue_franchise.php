...
try {
  ...
  $stmtF = $db->prepare("INSERT INTO franchises
                         (application_id, issue_date, expiry_date, status, certificate_no, approved_units, issued_at, issued_by_user_id, issued_by_name)
                         VALUES (?, ?, ?, 'Active', ?, ?, NOW(), ?, ?)");
  if (!$stmtF) throw new Exception('db_prepare_failed');
  $stmtF->bind_param('isssiis', $appId, $issueDate, $expiry, $certificateNo, $approvedUnits, $issuedByUserId, $issuedByName);
  if (!$stmtF->execute()) throw new Exception('insert_failed');
  $stmtF->close();
 
  $stmtU = $db->prepare("UPDATE franchise_applications
                         SET status='Active',
                             approved_vehicle_count=COALESCE(approved_vehicle_count, vehicle_count),
                             approved_at=NOW(),
                             approved_by_user_id=?,
                             approved_by_name=?
                         WHERE application_id=?");
  if (!$stmtU) throw new Exception('db_prepare_failed');
  $stmtU->bind_param('isi', $issuedByUserId, $issuedByName, $appId);
  $stmtU->execute();
  $stmtU->close();
 
  $db->commit();
  tmm_audit_event($db, 'FRANCHISE_ISSUED', 'Franchise', (string)$appId, ['certificate_no' => $certificateNo, 'issue_date' => $issueDate, 'expiry_date' => $expiry, 'approved_units' => $approvedUnits]);
  echo json_encode(['ok' => true, 'status' => 'Active', 'certificate_no' => $certificateNo, 'issue_date' => $issueDate, 'expiry_date' => $expiry, 'approved_units' => $approvedUnits]);
} catch (Throwable $e) {
  $db->rollback();
